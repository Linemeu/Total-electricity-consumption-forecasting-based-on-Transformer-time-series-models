...
data_month_ele = data_month_ele.loc[data_month_ele["date"]<="2013-09-30", :]
data_month_ele.reset_index(inplace=True, drop=True)
data_month = pd.merge(data_month_ele, data_month_exo, on="date", how="left")
data_month=data_month.loc[data_month["date"]<="2013-09-30", :]
...
data_month_train=data_month.loc[data_month["date"]<="2012-12-31", :]
data_month_test=data_month.loc[data_month["date"]>"2012-12-31", :]
...
for epoch in range(1000):
    ...
    for i in range(0, len(train_data_enc)):
        ...
        # enc=torch.concat([enc,enc_mark[i,:,:].unsqueeze(0)],dim=2).to(device)
...
S2_result=[mape(inv_truth_squence, inv_predict_squence),
           mse(inv_truth_squence, inv_predict_squence),
           mae(inv_truth_squence, inv_predict_squence)]
import pandas as pd
try:
    truth_predict_data=pd.read_excel('.\\truth_predict_data_S2.xlsx')
except:
    print('没找到文件')
    truth_predict_data=pd.DataFrame()
truth_predict_data['S2_Transformer_t2v_truth']=inv_truth_squence
truth_predict_data['S2_Transformer_t2v_predict']=inv_predict_squence
truth_predict_data.to_excel('truth_predict_data_S2.xlsx')
# import json
# with open('result_dict.json', 'r') as json_file:
# 	    result_dict = json.load(json_file)
# result_dict['S2_Transformer_mape']=float(S2_result[0])
# result_dict['S2_Transformer_mse']=float(S2_result[1])
# result_dict['S2_Transformer_mae']=float(S2_result[2])
# #mape,mse,mae
...
S1_result=[mape(inv_truth_squence, inv_predict_squence),
           mse(inv_truth_squence, inv_predict_squence),
           mae(inv_truth_squence, inv_predict_squence)]
result_dict['S2_Transformer_mape']=float(S1_result[0])
result_dict['S2_Transformer_mse']=float(S1_result[1])
result_dict['S2_Transformer_mae']=float(S1_result[2])
#mape,mse,mae
dict_json=json.dumps(result_dict)
with open('.\models\\result_dict_no_t2v.json','w+') as file:
    file.write(dict_json)
