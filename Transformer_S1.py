...
import pandas as pd
try:
    truth_predict_data=pd.read_excel('.\\truth_predict_data.xlsx')
except:
    print('没找到文件')
    truth_predict_data=pd.DataFrame()
truth_predict_data['S1_Transformer_t2v_truth']=inv_truth_squence
truth_predict_data['S1_Transformer_t2v_predict']=inv_predict_squence
truth_predict_data.drop(columns='Unnamed: 0')
truth_predict_data.to_excel('truth_predict_data.xlsx')
import json
try:
...
S1_result=[mape(inv_truth_squence, inv_predict_squence),
           mse(inv_truth_squence, inv_predict_squence),
           mae(inv_truth_squence, inv_predict_squence)]
result_dict['S1_Transformer_mape']=float(S1_result[0])
result_dict['S1_Transformer_mse']=float(S1_result[1])
result_dict['S1_Transformer_mae']=float(S1_result[2])
#mape,mse,mae
dict_json=json.dumps(result_dict)
