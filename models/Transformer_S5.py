...
inv_predict_squence=[inverse_normalized(test_predict_squence[k]) for k in range(len(test_predict_squence))]
inv_truth_squence=[inverse_normalized(test_truth_squence[k]) for k in range(len(test_truth_squence))]
...
S5_result=[mape(inv_truth_squence, inv_predict_squence),
           mse(inv_truth_squence, inv_predict_squence),
           mae(inv_truth_squence, inv_predict_squence)]

import pandas as pd
try:
    truth_predict_data=pd.read_excel('E:\用电量预测\\truth_predict_data_S5.xlsx')
except:
    print('没找到文件')
    truth_predict_data=pd.DataFrame()
truth_predict_data['S5_Transformer_t2v_truth']=inv_truth_squence
truth_predict_data['S5_Transformer_t2v_predict']=inv_predict_squence
truth_predict_data.to_excel('E:\用电量预测\\truth_predict_data_S5.xlsx')

import json
try:
    with open('E:\用电量预测\\result_dict_no_t2v.json', 'r') as json_file:
	    result_dict = json.load(json_file)
except:
        result_dict={}
result_dict['S5_Transformer_mape']=float(S5_result[0])
result_dict['S5_Transformer_mse']=float(S5_result[1])
result_dict['S5_Transformer_mae']=float(S5_result[2])
#mape,mse,mae
