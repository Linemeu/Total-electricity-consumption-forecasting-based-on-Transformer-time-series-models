...
data_month_ele = data_month_ele.loc[data_month_ele["date"]<="2016-02-29", :]
data_month_ele.reset_index(inplace=True, drop=True)
data_month = pd.merge(data_month_ele, data_month_exo, on="date", how="left")
data_month=data_month.loc[data_month["date"]<="2016-02-29", :]
...
data_month_train=data_month.loc[data_month["date"]<="2014-12-31", :]
data_month_test=data_month.loc[data_month["date"]>"2014-12-31", :]
...
S3_result=[mape(inv_truth_squence, inv_predict_squence),
           mse(inv_truth_squence, inv_predict_squence),
           mae(inv_truth_squence, inv_predict_squence)]
import pandas as pd
try:
    truth_predict_data=pd.read_excel('.\\truth_predict_data_S3.xlsx')
except:
    print('没找到文件')
    truth_predict_data=pd.DataFrame()
truth_predict_data['S3_Transformer_t2v_truth']=inv_truth_squence
truth_predict_data['S3_Transformer_t2v_predict']=inv_predict_squence
truth_predict_data.to_excel('truth_predict_data_S3.xlsx')
# import json
# with open('E:\用电量预测\\result_dict.json', 'r') as json_file:
# 	    result_dict = json.load(json_file)
# result_dict['S3_Transformer_mape']=float(S3_result[0])
# result_dict['S3_Transformer_mse']=float(S3_result[1])
# result_dict['S3_Transformer_mae']=float(S3_result[2])
# #mape,mse,mae
