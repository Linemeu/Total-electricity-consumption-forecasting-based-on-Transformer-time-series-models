...
import pandas as pd
try:
    truth_predict_data=pd.read_excel('.\\truth_predict_data_S5.xlsx')
except:
    print('没找到文件')
    truth_predict_data=pd.DataFrame()
truth_predict_data['S5_MLP_truth']=inv_truth_squence
truth_predict_data['S5_MLP_predict']=inv_predict_squence
truth_predict_data.to_excel('truth_predict_data_S5.xlsx')
...
result_dict['S5_MLP_mape'] = float(S5_result[0])
result_dict['S5_MLP_mse'] = float(S5_result[1])
result_dict['S5_MLP_mae'] = float(S5_result[2])
# mape,mse,mae
