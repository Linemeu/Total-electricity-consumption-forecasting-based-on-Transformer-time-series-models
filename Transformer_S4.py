...
data_month_ele = data_month_ele.loc[data_month_ele["date"]<="2018-04-30", :]
data_month_ele.reset_index(inplace=True, drop=True)
data_month = pd.merge(data_month_ele, data_month_exo, on="date", how="left")
data_month=data_month.loc[data_month["date"]<="2018-04-30", :]
...
data_month_train=data_month.loc[data_month["date"]<="2016-12-31", :]
data_month_test=data_month.loc[data_month["date"]>"2016-12-31", :]
...
S4_result=[mape(inv_truth_squence, inv_predict_squence),
           mse(inv_truth_squence, inv_predict_squence),
           mae(inv_truth_squence, inv_predict_squence)]
...
# result_dict['S4_Transformer_mape']=float(S4_result[0])
# result_dict['S4_Transformer_mse']=float(S4_result[1])
# result_dict['S4_Transformer_mae']=float(S4_result[2])
# #mape,mse,mae
...
truth_predict_data['S4_Transformer_t2v_truth']=inv_truth_squence
truth_predict_data['S4_Transformer_t2v_predict']=inv_predict_squence
